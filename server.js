    ...
    try {
        ...
        await db.tx(async t => {
            const patientId = await t.one(`
                INSERT INTO patient (
                    title_name, first_name, last_name, id_card, phone, gender, age, date_birth,
                    house_number, street, village, subdistrict, district, province, zipcode, 
...
app.put('/patients_web/:id', async (req, res) => {
    const patientId = req.params.id; // รับ id ของผู้ป่วยจาก URL
    const {
        title_name, first_name, last_name, id_card, phone, gender, date_birth,
        house_number, street, village, subdistrict, district, province, weight, height, waist, password
    } = req.body; // ข้อมูลใหม่ที่รับมาจากการแก้ไข

    try {
        // ตรวจสอบว่าผู้ป่วยที่ต้องการแก้ไขมีอยู่หรือไม่
        const existingPatient = await db.oneOrNone('SELECT * FROM patient WHERE id = $1', [patientId]);
        ...
        await db.none(`
            UPDATE patient
            SET 
                title_name = $1, first_name = $2, last_name = $3, id_card = $4, phone = $5, gender = $6,
                date_birth = $7, house_number = $8, street = $9, village = $10, subdistrict = $11,
                district = $12, province = $13, weight = $14, height = $15, waist = $16, password = $17
            WHERE id = $18
        `, [
            title_name, first_name, last_name, id_card, phone, gender, date_birth,
            house_number, street, village, subdistrict, district, province, weight, height, waist, password, patientId
        ]);
        ...
        await db.tx(async t => {
            ...
            await t.none(`
                ...
                SET
                    ...
                    district = $[district],
                    province = $[province],
                    zipcode = $[zipcode]  -- ลบจุลภาคหลัง zipcode
                WHERE id = $[patientId]
...
app.get('/zipcode', (req, res) => {
    ...
    // ตรวจสอบว่ามีข้อมูลครบทั้ง 3 อย่างหรือไม่
    if (!province || !district || !subdistrict) {
        return res.status(400).json({ message: 'กรุณาระบุ province, district และ subdistrict ให้ครบถ้วน' });
    }

    // ค้นหา zipcode จากฐานข้อมูล
    const result = thaiDatabase.find(data =>
        data.province === province &&
        data.amphoe === district &&
        data.district === subdistrict
    );

    if (result) {
        res.json({ zipcode: result.zipcode });
    } else {
        res.status(404).json({ message: 'ไม่พบข้อมูลที่ตรงกับ province, district และ subdistrict ที่ระบุ' });
    }
});
...
app.get('/appointments/details2/:id', async (req, res) => {
    ...
    try {
      const appointmentDetails = await db.one(`
        SELECT 
          p.title_name,        -- เพิ่ม title_name
          p.first_name, 
          p.last_name, 
          p.phone, 
          p.id_card, 
          p.date_birth, 
          p.gender,            -- เพิ่ม gender
          p.age,               -- เพิ่ม age
          p.zipcode,           -- เพิ่ม zipcode
          p.house_number, 
          p.street, 
          p.village, 
          p.subdistrict, 
...
app.put('/admin/update/:id', async (req, res) => {
    ...
    try {
        ...
        const existingAdmin = await db.oneOrNone('SELECT * FROM admins_web WHERE id = $1', [id]);

        if (!existingAdmin) {
            return res.status(404).json({ message: 'Admin not found' });
        }

        // ตรวจสอบว่า username ใหม่ซ้ำกับของ admin คนอื่นหรือไม่
        const usernameExists = await db.oneOrNone('SELECT * FROM admins_web WHERE username = $1 AND id != $2', [username, id]);

        if (usernameExists) {
            return res.status(400).json({ message: 'Username already exists' });
        }

        // ถ้า password ถูกส่งมาใหม่ ก็ทำการ hash ใหม่
        ...
        await db.none(`
            UPDATE admins_web 
            SET title_name = $1, first_name = $2, last_name = $3, username = $4, password = $5, plain_password = $6 
            WHERE id = $7`,
            [title_name, first_name, last_name, username, hashedPassword, password || existingAdmin.plain_password, id]);
...
app.get('/appointments/summary', async (req, res) => {
    try {
        const appointments = await db.any(`
            SELECT 
                a.id, 
            ...
            JOIN 
                patient p ON u.id_card = p.id_card
            ORDER BY 
                a.appointment_date ASC
        `);
